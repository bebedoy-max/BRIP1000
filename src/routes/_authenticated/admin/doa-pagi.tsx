...
export const Route = createFileRoute("/_authenticated/admin/doa-pagi")({
  head: () => ({
    meta: [
...
/** Gulirkan hanya daftar pekerja, tanpa ikut menggeser header atau layar bagian. */
function centerInputInAttendanceBody(input: HTMLInputElement) {
  const row = input.closest<HTMLElement>(".doa-row-zoom");
  const body = input.closest<HTMLElement>(".doa-body");
  if (!row || !body) return;
  ...
  const top =
    ...
    (body.clientHeight - rowRect.height) / 2;
  body.scrollTo({ top: Math.max(0, top), behavior: "smooth" });
}

/** Pindah ke bagian berikutnya tanpa menyerahkan pengguliran ke seluruh halaman. */
function scrollToAttendanceScreen(input: HTMLInputElement) {
  const root = input.closest<HTMLElement>(".doa-root");
  const screen = input.closest<HTMLElement>(".doa-screen");
  if (!root || !screen) return;

  const rootRect = root.getBoundingClientRect();
  const screenRect = screen.getBoundingClientRect();
  root.scrollTo({
    top: root.scrollTop + screenRect.top - rootRect.top,
    behavior: "smooth",
  });
}
          ...
          {section.pekerja.map((nama, row) => {
            ...
            return (
              <div
                ...
                data-next={!solo && idx === activeIdx + 1 ? "true" : undefined}
              >
...
function Page() {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ...
      if (!first) return;
      // Cegah browser menggulir wadah layar penuh saat input mendapat fokus.
      first.focus({ preventScroll: true });
      centerInputInAttendanceBody(first);
    }, 300);
    return () => window.clearTimeout(t);
  }, [started]);
  ...
  if (!savedToday && !started)
    return (
      <div className="doa-root doa-root-static">
        <button
          type="button"
          onClick={closeUker}
          className="doa-close"
        ...
        sections.map((s) => (
          <SectionScreen
            ...
            focusNext={(idx) => {
              ...
              const go = () => {
                // Pengguliran dikendalikan secara terpisah agar bagian tetap
                // berhenti tepat di atas dan header tidak ikut bergeser.
                next.focus({ preventScroll: true });
                next.select();
                // Hanya isi bagian yang bergulir; header bagian tetap diam.
                centerInputInAttendanceBody(next);
              };
              if (sameSection) {
                go();
                return;
